                ...
                else if (this.ModifiedClient == null)
                {
                    ...
                    using (var context = new DB_Entities())
                    {
                        try
                        {
                            ...
                            Globals.AllClients.Add(this.ModifiedClient);
                        }
                        catch (Exception ex)
                        {
                ...
                else
                {
                    ...
                    using (var context = new DB_Entities())
                    {
                        try
                        {
                            ...
                            foreach (tbl_clients client in Globals.AllClients)
                            {
                                if(client.ID == this.ModifiedClient.ID)
                                {
                                    client.name = this.ModifiedClient.name;
                                    client.person_responsible = this.ModifiedClient.person_responsible;
                                    client.status = this.ModifiedClient.status;
                                    client.english_name = this.ModifiedClient.english_name;
                                }
                            }
                        }
                        catch (Exception ex)
